...
void MainFrame::OnRegionSelected(wxCommandEvent& event) {
    ...
    if(regionsDropDownSelection > 0) {
        ...
        int regionCodeInt = getRegionCodeByCity(selectedCityStr);
        regionCode = new wxString(wxString::Format(wxT("%d"), regionCodeInt));
        std::cout << "regionCode: " << regionCode << "selectedCityStr" << selectedCityStr << std::endl;
    } else {
...
int MainFrame::getRegionCodeByCity(std::string cityName) {
    ...
    curl = curl_easy_init();
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_URL, "https://api.e-control.at/sprit/1.0/regions?includeCities=true");
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WriteCallback);
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, &readBuffer);
